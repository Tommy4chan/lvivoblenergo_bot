...
from bot.utils import get_weekday
...
def get_schedule_menu(user_id, selected_weekday):
    ...
    if current_weekday == selected_weekday:
        name_of_weekday_short[current_weekday] = f"{name_of_weekday_short[current_weekday]} ✅"
    else:
        if user_notification_state:
            name_of_weekday_short[current_weekday] = f"{name_of_weekday_short[current_weekday]} ☑️"
        name_of_weekday_short[selected_weekday] = f"{name_of_weekday_short[selected_weekday]} ✅"
    ...
    schedule_menu.row(InlineKeyboardButton(text=f'Сповіщення {"✅" if user_notification_state else "☑️"}', callback_data=f'notification_{user_notification_state}'), 
